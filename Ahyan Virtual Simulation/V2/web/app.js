(function () {
  ...
  const overlayLegendText = {
    ...
    compression: ["Compression Residual", "free XY", "blocked XY"],
    inducedHeight: ["Constraint-Induced Height", "flat/free", "lifted by constraint"],
  ...
  let selfCheckController = null;

  function setStatus(message) {
    if (RAD.Primitives?.setText) RAD.Primitives.setText(status, message);
  ...
  function setActiveView(mode) {
    document.querySelectorAll("[data-view-mode]").forEach((button) => {
      ...
      if (RAD.Primitives?.setPressed) RAD.Primitives.setPressed(button, active);
      else {
  ...
  function syncViewChrome() {
    document.body.classList.toggle("is-sheet-view", (state.view.cellVisualMode || "abstract") === "sheetOnly");
  }

  function applyWorkspaceMode(mode) {
    workspaceMode = ["inspect", "edit", "sheet", "analyze"].includes(mode) ? mode : "inspect";
    ...
    if (workspaceMode === "inspect") {
      ...
      Object.assign(view, {
        ...
        actuatorsVisible: false,
        actuatorDisplayMode: "selected",
        influenceFootprintVisible: false,
  ...
  function setupControlPanelSections() {
    ...
    if (RAD.Primitives?.installCollapsibleSections) {
      RAD.Primitives.installCollapsibleSections({ initiallyCollapsed });
      return;
    }
  ...
  function updateSelectionHud() {
    ...
    const constrained = sim.compressionResidual?.[r]?.[c] > 1e-6 ? " constrained" : "";
    selectionHud.state.textContent = removed ? "removed" : locked ? `locked${constrained}` : allowed ? `free${constrained}` : `masked${constrained}`;
  }
  ...
  try {
    ...
    selfCheckController = RAD.Primitives?.installSelfCheckPanel
      ? RAD.Primitives.installSelfCheckPanel({
          getContext: () => ({ renderer, state, sim }),
        })
      : null;
    ...
    document.getElementById("modeSheet").addEventListener("click", () => applyWorkspaceMode("sheet"));
    ...
    window.RAD_APP = {
      ...
      runSelfCheck: () =>
        selfCheckController
          ? selfCheckController.run()
          : RAD.Primitives?.runSelfCheck({ renderer, state, sim }),
