...
class LatticeState:
    ...
    lock_z_grid: np.ndarray | None = None
    ...
    def uniform(cls, config: LatticeConfig, alpha: float | None = None) -> "LatticeState":
        ...
        return cls(
            ...
            removed_mask=np.zeros(shape, dtype=bool),
            position_locked_mask=np.zeros(shape, dtype=bool),
            lock_z_grid=np.zeros(shape, dtype=float),
        )
    ...
    def normalized(self, config: LatticeConfig) -> "LatticeState":
        ...
        removed = (
            ...
            else np.asarray(self.removed_mask, dtype=bool)
        )
        position_locked = (
            np.zeros(shape, dtype=bool)
            if self.position_locked_mask is None
            else np.asarray(self.position_locked_mask, dtype=bool)
        )
        lock_z = (
            np.zeros(shape, dtype=float)
            if self.lock_z_grid is None
            else np.asarray(self.lock_z_grid, dtype=float)
        )
        ...
        if removed.shape != shape:
            raise ValueError(f"removed_mask must have shape {shape}")
        if position_locked.shape != shape:
            raise ValueError(f"position_locked_mask must have shape {shape}")
        if lock_z.shape != shape:
            raise ValueError(f"lock_z_grid must have shape {shape}")
        return LatticeState(
            alpha_grid=alpha.copy(),
            locked_mask=locked.copy(),
            actuator_grid=actuators.copy(),
            z_actuator_grid=z_actuators.copy(),
            removed_mask=removed.copy(),
            position_locked_mask=position_locked.copy(),
            lock_z_grid=lock_z.copy(),
        )
