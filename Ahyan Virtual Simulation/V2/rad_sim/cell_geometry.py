...
HARDWARE_PROFILE_SCHEMA = "rad-sim.hardware-profile.v1"
...
class RADHardwareProfile:
    ...
    def to_dict(self) -> dict[str, object]:
        return {
            ...
            "units": "mm",
            "dimensionsMm": {
                "sideLengthMm": self.side_length_mm,
                "fabricationHoleToleranceMm": self.fabrication_hole_tolerance_mm,
                "backlashMm": self.backlash_mm,
                "pinRadiusMm": self.pin_radius_mm,
                "holeRadiusMm": self.hole_radius_mm,
                "plateThicknessMm": self.plate_thickness_mm,
                "jointStackHeightMm": self.joint_stack_height_mm,
                "bossRadiusMm": self.boss_radius_mm,
            },
            "derived": {
                "pinHoleClearanceMm": self.pin_hole_clearance_mm,
            },
            "measuredFields": list(self.measured_fields),
    ...
    @classmethod
    def from_dict(cls, payload: dict[str, object]) -> "RADHardwareProfile":
        schema = payload.get("schema")
        if schema not in {None, HARDWARE_PROFILE_SCHEMA}:
            raise ValueError(f"unsupported hardware profile schema: {schema}")
        dimensions = payload.get("dimensionsMm", {})
        if not isinstance(dimensions, dict):
            raise ValueError("dimensionsMm must be an object")
        return cls(
            name=str(payload.get("name", "measured-hardware-profile")),
            source=str(payload.get("source", "external JSON")),
            side_length_mm=_profile_float(
                payload,
                dimensions,
                "sideLengthMm",
                "side_length_mm",
                PAPER_RAD_REFERENCE.side_length_mm,
            ),
            fabrication_hole_tolerance_mm=_profile_float(
                payload,
                dimensions,
            ...
            backlash_mm=_profile_optional_float(
                payload,
                dimensions,
                "backlashMm",
                "backlash_mm",
            ),
            pin_radius_mm=_profile_optional_float(
                payload,
                dimensions,
                "pinRadiusMm",
                "pin_radius_mm",
            ),
            hole_radius_mm=_profile_optional_float(
                payload,
                dimensions,
                "holeRadiusMm",
                "hole_radius_mm",
            ),
            plate_thickness_mm=_profile_optional_float(
                payload,
                dimensions,
                "plateThicknessMm",
                "plate_thickness_mm",
            ),
            joint_stack_height_mm=_profile_optional_float(
                payload,
                dimensions,
                "jointStackHeightMm",
                "joint_stack_height_mm",
            ),
            boss_radius_mm=_profile_optional_float(
                payload,
                dimensions,
                "bossRadiusMm",
                "boss_radius_mm",
...
def _profile_optional_float(
    payload: dict[str, object],
    dimensions: dict[object, object],
    camel_key: str,
    snake_key: str,
) -> float | None:
    value = dimensions.get(camel_key, payload.get(snake_key, None))
...
def _profile_float(
    ...
    value = dimensions.get(camel_key, payload.get(snake_key, default))
    return float(value)


def hardware_profile_from_dict(payload: dict[str, object]) -> RADHardwareProfile:
    return RADHardwareProfile.from_dict(payload)


def hardware_profile_from_json(text: str) -> RADHardwareProfile:
    payload = json.loads(text.lstrip("\ufeff"))
    if not isinstance(payload, dict):
        raise ValueError("hardware profile JSON must contain an object")
    return hardware_profile_from_dict(payload)


def export_hardware_profile_json(profile: RADHardwareProfile) -> str:
