...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Compare filled two-cell RAD fidelity-matrix measurements against "
            "the A360 CAD-derived reduced proxy."
        )
    )
    parser.add_argument("--input", required=True, help="Filled fidelity-matrix measurement CSV or JSON.")
    parser.add_argument("--out", default="outputs/two_cell_fidelity_matrix_measurement_comparison")
    ...
    files = write_two_cell_fidelity_matrix_measurement_comparison_artifacts(
        args.input,
        args.out,
