...
def relax_to_curve(target_curve, flat_line, max_iters=2000, step_size=0.01, tol=1e-6,
    ...
    T, cum, Ltot = build_arclength_map(target)   
    n = len(current)
    ...
    for it in range(max_iters):
        ...
        # Step 1 (arclength-mapped): move each node toward the target point
        # at the same normalized arclength position.
        for i in range(1, n ):
            u = i / (n )                # normalized node index in [0,1]
            s = u * Ltot                   # target arclength
            t = point_on_polyline_by_arclength(T, cum, s)
...
def ribbon_from_curve(x, y, z, sheet_width=0.4, n_strips=35, translate=(0.0, 0.0, 0.0), twist_per_width=0.0):
    ...
    return X, Y, Z
def build_arclength_map(target):
...
def point_on_polyline_by_arclength(T, cum, s):
    # find segment where cum[j] <= s <= cum[j+1]
...
if __name__ == "__main__":
    ...
    Xs, Ys, Zs = ribbon_from_curve(
        ...
        sheet_width=0.8,        # adjust width
        n_strips=25,            # adjust line density
