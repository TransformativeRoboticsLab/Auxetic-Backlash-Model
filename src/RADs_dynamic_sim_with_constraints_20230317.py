...
def animate_lattice(trajectory, bolt_cells_1based, servo_cells_1based, interval=30):
    ...
    # Colormap for face shading by z-height
    cmap = plt.get_cmap('coolwarm')
    z_min, z_max = -0.3, 0.3

    def make_quads(P):
        """Return list of quad vertices [(4,3), ...] for every cell in the lattice."""
        quads = []
        for i in range(Nu - 1):
            for j in range(Nv - 1):
                # Four corners of the (i,j) cell, wound CCW from above
                v0 = P[i,     j,     :]
                v1 = P[i + 1, j,     :]
                v2 = P[i + 1, j + 1, :]
                v3 = P[i,     j + 1, :]
    ...
    def face_colors(P):
        """Colour each quad by the mean z of its four corners."""
        colors = []
        for i in range(Nu - 1):
            for j in range(Nv - 1):
                z_mean = P[i:i+2, j:j+2, 2].mean()
                t = (z_mean - z_min) / (z_max - z_min)
                colors.append(cmap(np.clip(t, 0, 1)))
        return colors
    ...
    def update(frame):
        ...
        quads  = make_quads(P)
        fcolors = face_colors(P)
        poly = Poly3DCollection(quads, zsort='average', antialiased=False)
        poly.set_facecolor(fcolors)
        poly.set_edgecolor('0.25')   # dark grey edges between cells
        poly.set_linewidth(0.4)
        poly.set_alpha(0.85)
        ax.add_collection3d(poly)

        # Servo / bolt node overlays
        if servo_idx.size > 0:
            ax.scatter(P[servo_idx[:, 0], servo_idx[:, 1], 0],
                       P[servo_idx[:, 0], servo_idx[:, 1], 1],
                       P[servo_idx[:, 0], servo_idx[:, 1], 2],
                       s=60, c='red', zorder=5, label='Servo (Actuated)')
        if bolt_idx.size > 0:
            ax.scatter(P[bolt_idx[:, 0], bolt_idx[:, 1], 0],
                       P[bolt_idx[:, 0], bolt_idx[:, 1], 1],
                       P[bolt_idx[:, 0], bolt_idx[:, 1], 2],
                       s=60, c='orange', zorder=5, label='Bolt (Fixed)')
